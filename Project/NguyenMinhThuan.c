...
void xoaSachTheoMa(book a[], int *n);
...
void chuanHoa(char *name) {
    ...
    for (i = 0; i < strlen(name); i++) {
        if (isspace(name[i])) {//kiểm tra khoảng trắng
            chuDau = 1;  // Đặt lại khi gặp dấu cách
        } else {
            if (chuDau) {
                if (isalpha(name[i])) {//kiểm tra alphebet
...
void add(book a[], int *n) {
    ...
    for (; continueAdding == 1; (*n)++) {
        ...
        // Thêm tên sách
        do {
            printf("Nhap ten sach (it nhat 10 ky tu): ");
            scanf(" %[^\n]s", a[*n].tenSach);
        } while (strlen(a[*n].tenSach) < 10);

        chuanHoa(a[*n].tenSach);
        // Thêm tác giả
